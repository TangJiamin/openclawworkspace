...
// API URL（支持智谱 AI）
const GEMINI_API_URL = 'https://open.bigmodel.cn/api/paas/v4/chat/completions';
...
async function callGemini(prompt: string, apiKey: string): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`智谱 API error (${response.status}): ${errorText}`);
  }
...
function createAIClient(config: {
  ...
  const state = {
    geminiApiKey: config.geminiApiKey?.trim() || config.zhipu_api_key?.trim() || '',
    openaiApiKey: config.openaiApiKey?.trim() || '',
    openaiApiBase: (config.openaiApiBase?.trim() || OPENAI_DEFAULT_API_BASE).replace(/\/+$/, ''),
    openaiModel: config.openaiModel?.trim() || '',
    geminiEnabled: Boolean(config.geminiApiKey?.trim() || config.zhipu_api_key?.trim()),
  ...
  return {
    async call(prompt: string): Promise<string> {
      ...
      throw new Error('No AI API key configured. Set ZHIPU_API_KEY or OPENAI_API_KEY in environment or config.json. For ZHIPU_API_KEY, get one at https://open.bigmodel.cn/');
    },
  };
}
...
Environment:
  ZHIPU_API_KEY   推荐使用智谱 AI GLM-4-Plus。Get one at https://open.bigmodel.cn/
...
async function main(): Promise<void> {
  ...
  const geminiApiKey = process.env.ZHIPU_API_KEY || process.env.GEMINI_API_KEY;
  const openaiApiKey = process.env.OPENAI_API_KEY;
  const openaiApiBase = process.env.OPENAI_API_BASE;
  const openaiModel = process.env.OPENAI_MODEL;

  if (!geminiApiKey && !openaiApiKey) {
    console.error('[digest] Error: Missing API key. Set ZHIPU_API_KEY (推荐) or OPENAI_API_KEY.');
